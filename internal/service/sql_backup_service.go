...
// the schema-agnostic sqldump engine over scheduler's own tables (from ent
// migrate.Tables). Replaces the legacy per-field scheduler.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("scheduler/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "scheduler", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("scheduler: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "scheduler", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "scheduler", Warnings: res.Warnings}
